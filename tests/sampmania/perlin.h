...
class perlin 
{
    ...
    {
        return vec3f({ (float)maths::random_double(min, max), (float)maths::random_double(min, max), (float)maths::random_double(min, max) });
    }
...
public:
    perlin() {
        ...
        for (int i = 0; i < point_count; ++i) {
            ranvec[i] = perlin::random(-1, 1).unit();
        }
...
private:
    ...
    static void permute(int* p, int n) {
        for (int i = n - 1; i > 0; i--) {
            int target = maths::random_int(0, i);
            int tmp = p[i];
            p[i] = p[target];
            p[target] = tmp;
        }
    ...
    static double perlin_interp(vec3f c[2][2][2], double u, double v, double w) 
    {
        ...
        for (int i = 0; i < 2; i++)
            for (int j = 0; j < 2; j++)
                for (int k = 0; k < 2; k++) {
                    vec3f weight_v({ (float)u - i, (float)v - j, (float)w - k });
                    accum += (i * uu + (1 - i) * (1 - uu)) *
                        (j * vv + (1 - j) * (1 - vv)) *
