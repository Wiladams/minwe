...
namespace targa {
    ...
    static bool decodeSinglePixel(PixelRGBA& pix, uint8_t* databuff, int pixelDepth, int imtype, PixelRGBA* colorMap)
    {
    ...
    static PixelRGBA* readColorMap(BinStream& bs, tgaHeader& header)
    {
        size_t bytespe = header.CMapDepth / 8;     // bytes per entry
        uint8_t databuff[16];

        PixelRGBA* cMap = { new PixelRGBA[header.CMapLength]{} };
    ...
    public:
        ...
        PixelRGBA getCurrent() const { return pix; }
    };
    ...
    public:
        ...
        bool moveNext()
        {
            if (pixelCount == 0) {
                ...
                pixelCount = (int)winme::BITSVALUE(packet, 0, 6) + 1;
                //printf("pixelCount: %d\n", pixelCount);
